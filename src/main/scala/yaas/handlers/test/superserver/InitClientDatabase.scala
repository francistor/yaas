...
class InitClientDatabase(statsServer: ActorRef, configObject: Option[String]) extends MessageHandler(statsServer, configObject) {
  ...
  log.info("Re-Creating Clients database")

  try {
    ignite.destroyCache("CLIENTS")
  } catch {
    case _ : Throwable=>
      // Swallow this
  }

  val clientCache = ignite.getOrCreateCache[Nothing, Nothing](new org.apache.ignite.configuration.CacheConfiguration[Nothing, Nothing].setName("CLIENTS").setCacheMode(REPLICATED).setSqlSchema("CLIENTS"))
