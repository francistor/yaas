...
class AccessRequestHandler(statsServer: ActorRef, configObject: Option[String]) extends MessageHandler(statsServer, configObject) {
  ...
  private val dbConf = yaas.config.ConfigManager.getConfigObjectAsJson("clientsDatabase.json")
  ...
  def handleAccessRequest(implicit ctx: RadiusRequestContext): Unit = {
    ...
    def handlePrepaidRequest(implicit ctx: RadiusRequestContext): Unit = {
      ...
      log.debug("Handling Prepaid Request")
    ...
    def handleStdAccessRequest(implicit ctx: RadiusRequestContext): Unit = {
      ...
      log.debug("Handling Standard Access-Request")
